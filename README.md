...
Power on the robot, wait for initialization, and press the start button (if using `Xmotion_code.ino`). The robot will:
- Keep within ring boundaries using line sensors
- Detect and attack opponents using obstacle sensors
- Perform a star search pattern when opponent is lost
...
- `search_distance`: Search pattern distance (default: 100)
- `Timer`: Movement duration (default: 1500ms)

## Troubleshooting

- **Robot not moving**: Check motor connections and power supply
- **Sensors not detecting**: Verify pin connections and sensor power
- **Robot falls out of ring**: Calibrate line sensors and adjust thresholds
- **Not detecting opponents**: Check obstacle sensor connections and range
